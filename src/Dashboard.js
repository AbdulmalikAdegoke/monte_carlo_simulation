...
// 
import ProjectDescription from './Components/DB_ProjectDescription'
import TaskScheduling from './Components/DB_TaskScheduling'
import Output from './Components/DB_Output'

export default function Dashboard({ userProfile, projectTaskFormValidation, projectFormValidation }) {
  const [currentPage,setCurrentPage] = useState(0);
    ...
    const currentForm = (page) => {
      if (page === 0) {
        return <ProjectDescription formData={formData} setFormData={setFormData} projectFormValidation={projectFormValidation} />;
      } else if (page === 1) {
        return <TaskScheduling formData={formData} setFormData={setFormData} tasks={tasks} setTasks={setTasks} risks={risks} setRisks={setRisks} projectTaskFormValidation={projectTaskFormValidation} />
      } else {
